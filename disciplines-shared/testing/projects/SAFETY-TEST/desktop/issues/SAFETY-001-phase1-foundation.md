...
labels: ["testing", "safety", "1 — Foundation"]
blocked_by: []
depends_on: []
...
- [ ] CSS class naming follows `A-02400-*` convention

## Dependencies

- **Blocked by:** None
- **Depends on:** None
- **Blocks:** SAFETY-003, SAFETY-004, SAFETY-005, SAFETY-006, SAFETY-007, SAFETY-008
...
**Critical** — blocks all subsequent testing. A failure in any foundation component halts the entire SAFETY-TEST pipeline.

## QC Team Checks

The following checks must be performed against the PAGE-KNOWLEDGE metadata for the 02400 page:
...
- [ ] **Background image:** Confirm `background-image` CSS property resolves to a valid image URL (safety blue gradient); verify no 404 in network tab
- [ ] **CSS class naming:** All page-level elements use `A-02400-*` class prefix convention
