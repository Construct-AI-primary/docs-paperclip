---
title: Heartbeat Monitoring Configuration — ELEC-TEST Agent Feedback Loop
description: Configuration for real-time agent activity monitoring, stall detection, and escalation during ELEC-TEST execution
author: Paperclip QA Team
date: 2026-04-28
version: 1.0
---

# Heartbeat Monitoring Configuration — ELEC-TEST Agent Feedback Loop

## Overview

This document configures the agent feedback loop that enables real-time visibility into what agents are doing during ELEC-TEST execution. It leverages Paperclip's built-in ActivityEvent logging and heartbeat run tracking.

## Monitoring Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                    ELEC-TEST Feedback Loop                        │
│                                                                  │
│  Poller (15min)                                                  │
│  ├── GET /api/companies/qualityforge-ai/activity                │
│  │   └── ?entityType=issue&entityId=ELEC-*                      │
│  │                                                              │
...
monitoring:
  ...
  endpoints:
    ...
    issue_status:
      ...
      params:
        ids: ["ELEC-001", "ELEC-002", ..., "ELEC-014"]
  output_file: "ELEC-TEST-FEEDBACK-LOOP.md"
...
```markdown
# ELEC-TEST Agent Feedback Loop — {TIMESTAMP}

## Active Agents

| Agent | Issue | Status | Last Heartbeat | Tool Calls | Sub-Agents | Stall Risk |
|-------|-------|--------|----------------|------------|------------|------------|
| validator-qualityforge | ELEC-003 | in_progress | 2min ago | 14 | 0 | ✅ OK |
| electrical-engineering-domainforge-electrical | ELEC-007 | in_review | 8min ago | 7 | 0 | ✅ OK |
| electrical-engineering-domainforge-power-systems | ELEC-009 | backlog | never | 0 | 0 | ⚠ NOT STARTED |
| database-infraforge | ELEC-002 | done | 22min ago | 5 | 0 | ✅ OK |
...
```
GET /api/companies/qualityforge-ai/activity/runs?entityId=ELEC-003
```
...
DomainForge agent completes domain check
  ↓
Updates sub-task status → status propagates to parent (ELEC-XXX)
  ↓
validator-qualityforge sees status update via heartbeat poll
  ↓
Updates ELEC-TEST-FEEDBACK-LOOP.md dashboard
  ↓
...
### Before ELEC-TEST Execution

1. [ ] Ensure activity log API is accessible:
   ```bash
   curl http://localhost:3100/api/companies/qualityforge-ai/activity?limit=1
   ```
2. [ ] Verify ELEC-* issues exist in the system:
   ```bash
   curl http://localhost:3100/api/companies/qualityforge-ai/issues?search=ELEC
   ```
3. [ ] Configure heartbeat monitor script to poll every 15min
4. [ ] Create `ELEC-TEST-FEEDBACK-LOOP.md` initial file

### During ELEC-TEST Execution
