...
# QS-013a: Agent Feedback Loop — Live Monitoring, Stall Detection & Escalation

## Executive Summary
**Objective**: Configure and validate the agent feedback loop that provides real-time visibility into what agents are doing during QS-TEST execution. This answers the question "what are the agents doing?" by polling Paperclip's ActivityEvent API.
...
- **13aA**: Configure polling routine calling `GET /api/companies/qualityforge-ai/activity?entityType=issue&entityId=QS-*`
...
- QS-013 (Agent Delegation) — feedback loop monitors delegation results
