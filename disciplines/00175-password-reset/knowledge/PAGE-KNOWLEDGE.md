---
title: Password Reset Agent Architectural Knowledge
description: Comprehensive architectural knowledge of password reset system design, database schemas, API patterns, and compliance frameworks for password reset agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00175-password-reset
gigabrain_tags: knowledgeforge-ai, password-reset-agent, password-reset-architecture, system-design, compliance-frameworks
openstinger_context: password-reset-agent-architecture, password-reset-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00175_Password Reset/agent-data/domain-knowledge/00175_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00175_Password Reset/00175-password-reset-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/password-reset-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the password reset system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for password reset module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide password reset system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Password Reset Database Architecture
...
#### Password Reset Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE password_reset_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  password_reset_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE password_reset_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "password_reset_records_company_access" ON password_reset_records
...
CREATE INDEX idx_password_reset_records_password_reset_id ON password_reset_records(password_reset_id);
CREATE INDEX idx_password_reset_records_status ON password_reset_records(status);
CREATE INDEX idx_password_reset_records_company_id ON password_reset_records(company_id);
```
...
CREATE TABLE password_reset_details (
  ...
  value TEXT NOT NULL REFERENCES password_reset_records(password_reset_id),
...
CREATE INDEX CONCURRENTLY idx_password_reset_records_composite
ON password_reset_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_password_reset_records_active
ON password_reset_records(password_reset_id)
...
CREATE TABLE password_reset_records_y2026m01 PARTITION OF password_reset_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed password reset records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Password Reset API Architecture
...
**Versioning**: URL-based versioning (`/api/v1/password-reset`)

#### Core API Endpoints

**Password Reset Record Management**:
```typescript
// GET /api/password-reset/password reset records
// List password reset records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/password-reset/password reset records
...
// PUT /api/password-reset/password reset records/{id}
...
const PERMISSIONS = {
  'create_password_resetRecord': ['password-reset_admin', 'password-reset_manager'],
  'approve_password_resetRecord': ['password-reset_admin'],
  'view_password_resetRecord': ['password-reset_user', 'password-reset_admin', 'password-reset_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Password Reset Record
const password_resetRecordSchema = z.object({
  password_reset_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM password_reset_records
  WHERE password_reset_id = ${userInput}
...
// Cache Keys Pattern
const CACHE_KEYS = {
  password_resetRecord_list: ({filters}) => `password-reset:password reset records:list:${hash(filters)}`,
  password_resetRecord_detail: (id) => `password-reset:password_resetRecord:${id}`,
  password_resetRecord_stats: () => `password-reset:password reset records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT password_reset_id, status, status, created_at
FROM password_reset_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_password_reset_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Password Reset Security Architecture
...
const RBAC_MATRIX = {
  'password-reset_admin': {
    create: ['password reset records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['password reset records', 'reports'],
    approve: ['password reset records', 'budgets']
  },
  'password-reset_manager': {
    create: ['password reset records', 'reports'],
    read: ['password reset records', 'reports', 'users'],
    update: ['password reset records', 'reports'],
    delete: [],
    approve: ['password reset records']
  },
  'password-reset_user': {
    create: ['password reset records'],
    read: ['password reset records', 'reports'],
    update: ['own_password reset records'],
    delete: [],
    approve: []
  }
};
...
- **Password Reset Creation**: active - Fully implemented with approval workflow integration
- **Password Reset Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Password Reset Creation
interface password_reset_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'password_reset_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in password_reset approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Password Reset Creation
...
### 4.1 Password Reset Creation Modal

**Modal ID**: `PasswordResetCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new password reset records
...
### 4.2 Password Reset Details Modal

**Modal ID**: `PasswordResetDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit password reset record details
...
**Purpose**: Store and retrieve password reset documents and attachments

**Integration Points**:
- **Document Upload**: Upload password reset related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/password_reset/documents/upload
/api/password_reset/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for password reset record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of password reset status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/password_reset/email/status-update
/api/password_reset/email/approval-request
```
...
#### Password Reset Table Component
**Component**: `Password ResetTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Password Reset Record Validation
```javascript
const password_resetRecordValidationRules = {
  password_resetId: {
    required: true,
...
- **Record Not Found**: "Password Reset record not found. Please check the record ID."
...
#### Lazy Loading
- Load password reset records and data on demand
- Paginate large password reset records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used password reset records and data for 24 hours
...
- Full-text search across password reset names and descriptions
- Fuzzy matching for record details
- Category-based filtering for password reset records
...
- **Primary Discipline**: 00175 Password Reset
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - password-reset-knowledgeforge
  - password-reset-knowledgeforge-password-reset
...
|----------|--------|---------|----------|
| `/api/password_reset/records` | GET | List password reset records | Array of record objects |
| `/api/password_reset/details` | GET | Get password reset record details | Record detail object |

### Appendix B: Common Password Reset Abbreviations
...
### Appendix C: Password Reset Record Number Format
...
**Examples**:
- 00175-2026-00001 (First password reset record of 2026)
- 00175-2026-01234 (1234th password reset record of 2026)
...
- Page Load State: `password_reset-baseline-page-load.png`
- Button Interactions: `password_reset-baseline-button-states.png`
- Modal Layouts: `password_reset-baseline-modals.png`
- Form States: `password_reset-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('password_reset-correspondence-btn');
