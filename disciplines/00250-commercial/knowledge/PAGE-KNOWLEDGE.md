---
title: Commercial Agent Architectural Knowledge
description: Comprehensive architectural knowledge of commercial system design, database schemas, API patterns, and compliance frameworks for commercial agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00250-commercial
gigabrain_tags: knowledgeforge-ai, commercial-agent, commercial-architecture, system-design, compliance-frameworks
openstinger_context: commercial-agent-architecture, commercial-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00250_Commercial/agent-data/domain-knowledge/00250_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00250_Commercial/00250-commercial-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/commercial-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the commercial system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for commercial module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide commercial system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Commercial Database Architecture
...
#### Commercial Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE commercial_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  commercial_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE commercial_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "commercial_records_company_access" ON commercial_records
...
CREATE INDEX idx_commercial_records_commercial_id ON commercial_records(commercial_id);
CREATE INDEX idx_commercial_records_status ON commercial_records(status);
CREATE INDEX idx_commercial_records_company_id ON commercial_records(company_id);
```
...
CREATE TABLE commercial_details (
  ...
  value TEXT NOT NULL REFERENCES commercial_records(commercial_id),
...
CREATE INDEX CONCURRENTLY idx_commercial_records_composite
ON commercial_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_commercial_records_active
ON commercial_records(commercial_id)
...
CREATE TABLE commercial_records_y2026m01 PARTITION OF commercial_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed commercial records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Commercial API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/commercial`
...
**Versioning**: URL-based versioning (`/api/v1/commercial`)

#### Core API Endpoints

**Commercial Record Management**:
```typescript
// GET /api/commercial/commercial records
// List commercial records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/commercial/commercial records
...
// PUT /api/commercial/commercial records/{id}
...
const PERMISSIONS = {
  'create_commercialRecord': ['commercial_admin', 'commercial_manager'],
  'approve_commercialRecord': ['commercial_admin'],
  'view_commercialRecord': ['commercial_user', 'commercial_admin', 'commercial_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Commercial Record
const commercialRecordSchema = z.object({
  commercial_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM commercial_records
  WHERE commercial_id = ${userInput}
...
// Cache Keys Pattern
const CACHE_KEYS = {
  commercialRecord_list: ({filters}) => `commercial:commercial records:list:${hash(filters)}`,
  commercialRecord_detail: (id) => `commercial:commercialRecord:${id}`,
  commercialRecord_stats: () => `commercial:commercial records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT commercial_id, status, status, created_at
...
-- Index Scan using idx_commercial_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Commercial Security Architecture
...
const RBAC_MATRIX = {
  'commercial_admin': {
    create: ['commercial records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['commercial records', 'reports'],
    approve: ['commercial records', 'budgets']
  },
  'commercial_manager': {
    create: ['commercial records', 'reports'],
    read: ['commercial records', 'reports', 'users'],
    update: ['commercial records', 'reports'],
    delete: [],
    approve: ['commercial records']
  },
  'commercial_user': {
    create: ['commercial records'],
    read: ['commercial records', 'reports'],
    update: ['own_commercial records'],
    delete: [],
    approve: []
  }
};
...
- **Commercial Creation**: active - Fully implemented with approval workflow integration
- **Commercial Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Commercial Creation
interface commercial_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'commercial_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in commercial approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Commercial Creation
...
### 4.1 Commercial Creation Modal

**Modal ID**: `CommercialCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new commercial records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00250-2026-00001]
...
### 4.2 Commercial Details Modal

**Modal ID**: `CommercialDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit commercial record details
...
**Purpose**: Store and retrieve commercial documents and attachments

**Integration Points**:
- **Document Upload**: Upload commercial related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/commercial/documents/upload
/api/commercial/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for commercial record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of commercial status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/commercial/email/status-update
/api/commercial/email/approval-request
```
...
#### Commercial Table Component
**Component**: `CommercialTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Commercial Record Validation
```javascript
const commercialRecordValidationRules = {
  commercialId: {
    required: true,
...
- **Record Not Found**: "Commercial record not found. Please check the record ID."
...
#### Lazy Loading
- Load commercial records and data on demand
- Paginate large commercial records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used commercial records and data for 24 hours
...
- Full-text search across commercial names and descriptions
- Fuzzy matching for record details
- Category-based filtering for commercial records
...
- **Primary Discipline**: 00250 Commercial
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - commercial-knowledgeforge
  - commercial-knowledgeforge-commercial
...
|----------|--------|---------|----------|
| `/api/commercial/records` | GET | List commercial records | Array of record objects |
| `/api/commercial/details` | GET | Get commercial record details | Record detail object |

### Appendix B: Common Commercial Abbreviations
...
### Appendix C: Commercial Record Number Format

**Format**: 00250-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00250-2026-00001 (First commercial record of 2026)
- 00250-2026-01234 (1234th commercial record of 2026)
...
**Container**: `A-00250-button-container`
...
- Page Load State: `commercial-baseline-page-load.png`
- Button Interactions: `commercial-baseline-button-states.png`
- Modal Layouts: `commercial-baseline-modals.png`
- Form States: `commercial-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('commercial-correspondence-btn');
