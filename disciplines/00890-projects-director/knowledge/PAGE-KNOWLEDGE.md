---
title: Projects Director Agent Architectural Knowledge
description: Comprehensive architectural knowledge of projects director system design, database schemas, API patterns, and compliance frameworks for projects director agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00890-projects-director
gigabrain_tags: knowledgeforge-ai, projects-director-agent, projects-director-architecture, system-design, compliance-frameworks
openstinger_context: projects-director-agent-architecture, projects-director-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00890_Projects Director/agent-data/domain-knowledge/00890_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00890_Projects Director/00890-projects-director-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/projects-director-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the projects director system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for projects director module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide projects director system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Projects Director Database Architecture
...
#### Projects Director Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE projects_director_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  projects_director_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE projects_director_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "projects_director_records_company_access" ON projects_director_records
...
CREATE INDEX idx_projects_director_records_projects_director_id ON projects_director_records(projects_director_id);
CREATE INDEX idx_projects_director_records_status ON projects_director_records(status);
CREATE INDEX idx_projects_director_records_company_id ON projects_director_records(company_id);
```
...
```sql
CREATE TABLE projects_director_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES projects_director_records(projects_director_id),
...
CREATE INDEX CONCURRENTLY idx_projects_director_records_composite
ON projects_director_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_projects_director_records_active
ON projects_director_records(projects_director_id)
...
CREATE TABLE projects_director_records_y2026m01 PARTITION OF projects_director_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed projects director records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Projects Director API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/projects-director`
...
**Versioning**: URL-based versioning (`/api/v1/projects-director`)

#### Core API Endpoints

**Projects Director Record Management**:
```typescript
// GET /api/projects-director/projects director records
// List projects director records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/projects-director/projects director records
...
// PUT /api/projects-director/projects director records/{id}
...
const PERMISSIONS = {
  'create_projects_directorRecord': ['projects-director_admin', 'projects-director_manager'],
  'approve_projects_directorRecord': ['projects-director_admin'],
  'view_projects_directorRecord': ['projects-director_user', 'projects-director_admin', 'projects-director_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Projects Director Record
const projects_directorRecordSchema = z.object({
  projects_director_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM projects_director_records
  WHERE projects_director_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'projects-director:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  projects_directorRecord_list: ({filters}) => `projects-director:projects director records:list:${hash(filters)}`,
  projects_directorRecord_detail: (id) => `projects-director:projects_directorRecord:${id}`,
  projects_directorRecord_stats: () => `projects-director:projects director records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT projects_director_id, status, status, created_at
FROM projects_director_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_projects_director_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Projects Director Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'projects-director_admin': {
    create: ['projects director records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['projects director records', 'reports'],
    approve: ['projects director records', 'budgets']
  },
  'projects-director_manager': {
    create: ['projects director records', 'reports'],
    read: ['projects director records', 'reports', 'users'],
    update: ['projects director records', 'reports'],
    delete: [],
    approve: ['projects director records']
  },
  'projects-director_user': {
    create: ['projects director records'],
    read: ['projects director records', 'reports'],
    update: ['own_projects director records'],
    delete: [],
    approve: []
  }
};
...
- **Projects Director Creation**: active - Fully implemented with approval workflow integration
- **Projects Director Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Projects Director Creation
interface projects_director_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'projects_director_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in projects_director approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Projects Director Creation
...
### 4.1 Projects Director Creation Modal

**Modal ID**: `ProjectsDirectorCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new projects director records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00890-2026-00001]
...
### 4.2 Projects Director Details Modal

**Modal ID**: `ProjectsDirectorDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit projects director record details
...
**Purpose**: Store and retrieve projects director documents and attachments

**Integration Points**:
- **Document Upload**: Upload projects director related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/projects_director/documents/upload
/api/projects_director/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for projects director record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of projects director status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/projects_director/email/status-update
/api/projects_director/email/approval-request
```
...
#### Projects Director Table Component
**Component**: `Projects DirectorTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Projects Director Record Validation
```javascript
const projects_directorRecordValidationRules = {
  projects_directorId: {
    required: true,
...
- **Record Not Found**: "Projects Director record not found. Please check the record ID."
...
#### Lazy Loading
- Load projects director records and data on demand
- Paginate large projects director records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used projects director records and data for 24 hours
...
- Full-text search across projects director names and descriptions
- Fuzzy matching for record details
- Category-based filtering for projects director records
...
#### Projects Director Workflow Testing
...
- **Primary Discipline**: 00890 Projects Director
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - projects-director-knowledgeforge
  - projects-director-knowledgeforge-projects-director
...
|----------|--------|---------|----------|
| `/api/projects_director/records` | GET | List projects director records | Array of record objects |
| `/api/projects_director/details` | GET | Get projects director record details | Record detail object |

### Appendix B: Common Projects Director Abbreviations

| Abbreviation | Full Term |
|-------------|-----------|
| 0089 | Projects Director |
| REC | Record |

### Appendix C: Projects Director Record Number Format

**Format**: 00890-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00890-2026-00001 (First projects director record of 2026)
- 00890-2026-01234 (1234th projects director record of 2026)
...
- Page Load State: `projects_director-baseline-page-load.png`
- Button Interactions: `projects_director-baseline-button-states.png`
- Modal Layouts: `projects_director-baseline-modals.png`
- Form States: `projects_director-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('projects_director-correspondence-btn');
