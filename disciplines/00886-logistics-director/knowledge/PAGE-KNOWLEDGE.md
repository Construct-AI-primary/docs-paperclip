---
title: Logistics Director Agent Architectural Knowledge
description: Comprehensive architectural knowledge of logistics director system design, database schemas, API patterns, and compliance frameworks for logistics director agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00886-logistics-director
gigabrain_tags: knowledgeforge-ai, logistics-director-agent, logistics-director-architecture, system-design, compliance-frameworks
openstinger_context: logistics-director-agent-architecture, logistics-director-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00886_Logistics Director/agent-data/domain-knowledge/00886_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00886_Logistics Director/00886-logistics-director-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/logistics-director-knowledgeforge/SKILL.md
---
...
This document provides the Director agent with comprehensive architectural knowledge of the logistics director system design, database schemas, API patterns, security frameworks, and compliance requirements. The Director agent serves as the architectural authority for logistics director module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Director agent to guide logistics director system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Logistics Director Database Architecture
...
#### Logistics Director Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE logistics_director_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  logistics_director_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE logistics_director_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "logistics_director_records_company_access" ON logistics_director_records
...
CREATE INDEX idx_logistics_director_records_logistics_director_id ON logistics_director_records(logistics_director_id);
CREATE INDEX idx_logistics_director_records_status ON logistics_director_records(status);
CREATE INDEX idx_logistics_director_records_company_id ON logistics_director_records(company_id);
```
...
```sql
CREATE TABLE logistics_director_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES logistics_director_records(logistics_director_id),
...
CREATE INDEX CONCURRENTLY idx_logistics_director_records_composite
ON logistics_director_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_logistics_director_records_active
ON logistics_director_records(logistics_director_id)
...
CREATE TABLE logistics_director_records_y2026m01 PARTITION OF logistics_director_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed logistics director records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Logistics Director API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/logistics-director`
...
**Versioning**: URL-based versioning (`/api/v1/logistics-director`)

#### Core API Endpoints

**Logistics Director Record Management**:
```typescript
// GET /api/logistics-director/logistics director records
// List logistics director records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/logistics-director/logistics director records
...
// PUT /api/logistics-director/logistics director records/{id}
...
const PERMISSIONS = {
  'create_logistics_directorRecord': ['logistics-director_admin', 'logistics-director_manager'],
  'approve_logistics_directorRecord': ['logistics-director_admin'],
  'view_logistics_directorRecord': ['logistics-director_user', 'logistics-director_admin', 'logistics-director_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Logistics Director Record
const logistics_directorRecordSchema = z.object({
  logistics_director_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM logistics_director_records
  WHERE logistics_director_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'logistics-director:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  logistics_directorRecord_list: ({filters}) => `logistics-director:logistics director records:list:${hash(filters)}`,
  logistics_directorRecord_detail: (id) => `logistics-director:logistics_directorRecord:${id}`,
  logistics_directorRecord_stats: () => `logistics-director:logistics director records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT logistics_director_id, status, status, created_at
FROM logistics_director_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_logistics_director_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Logistics Director Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'logistics-director_admin': {
    create: ['logistics director records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['logistics director records', 'reports'],
    approve: ['logistics director records', 'budgets']
  },
  'logistics-director_manager': {
    create: ['logistics director records', 'reports'],
    read: ['logistics director records', 'reports', 'users'],
    update: ['logistics director records', 'reports'],
    delete: [],
    approve: ['logistics director records']
  },
  'logistics-director_user': {
    create: ['logistics director records'],
    read: ['logistics director records', 'reports'],
    update: ['own_logistics director records'],
    delete: [],
    approve: []
  }
};
...
- **Logistics Director Creation**: active - Fully implemented with approval workflow integration
- **Logistics Director Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Logistics Director Creation
interface logistics_director_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'logistics_director_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in logistics_director approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Logistics Director Creation
...
### 4.1 Logistics Director Creation Modal

**Modal ID**: `LogisticsDirectorCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new logistics director records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00886-2026-00001]
...
### 4.2 Logistics Director Details Modal

**Modal ID**: `LogisticsDirectorDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit logistics director record details
...
**Purpose**: Store and retrieve logistics director documents and attachments

**Integration Points**:
- **Document Upload**: Upload logistics director related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/logistics_director/documents/upload
/api/logistics_director/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for logistics director record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of logistics director status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/logistics_director/email/status-update
/api/logistics_director/email/approval-request
```
...
#### Logistics Director Table Component
**Component**: `Logistics DirectorTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Logistics Director Record Validation
```javascript
const logistics_directorRecordValidationRules = {
  logistics_directorId: {
    required: true,
...
- **Record Not Found**: "Logistics Director record not found. Please check the record ID."
...
#### Lazy Loading
- Load logistics director records and data on demand
- Paginate large logistics director records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used logistics director records and data for 24 hours
...
- Full-text search across logistics director names and descriptions
- Fuzzy matching for record details
- Category-based filtering for logistics director records
...
#### Logistics Director Workflow Testing
...
- **Primary Discipline**: 00886 Logistics Director
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - logistics-director-knowledgeforge
  - logistics-director-knowledgeforge-logistics-director
...
|----------|--------|---------|----------|
| `/api/logistics_director/records` | GET | List logistics director records | Array of record objects |
| `/api/logistics_director/details` | GET | Get logistics director record details | Record detail object |

### Appendix B: Common Logistics Director Abbreviations

| Abbreviation | Full Term |
|-------------|-----------|
| 0088 | Logistics Director |
| REC | Record |

### Appendix C: Logistics Director Record Number Format

**Format**: 00886-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00886-2026-00001 (First logistics director record of 2026)
- 00886-2026-01234 (1234th logistics director record of 2026)
...
**Container**: `A-00886-button-container`
...
- Page Load State: `logistics_director-baseline-page-load.png`
- Button Interactions: `logistics_director-baseline-button-states.png`
- Modal Layouts: `logistics_director-baseline-modals.png`
- Form States: `logistics_director-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('logistics_director-correspondence-btn');
