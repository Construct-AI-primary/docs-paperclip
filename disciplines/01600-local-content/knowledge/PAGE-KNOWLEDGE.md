...
related_docs:
  ...
  - docs-construct-ai/disciplines/01600_Local Content/01600-local-content-page-documentation.md
...
This document provides the Specialist agent with comprehensive architectural knowledge of the local content system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for local content module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide local content system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Local Content Database Architecture
...
#### Local Content Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE local_content_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  local_content_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE local_content_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "local_content_records_company_access" ON local_content_records
...
CREATE INDEX idx_local_content_records_local_content_id ON local_content_records(local_content_id);
CREATE INDEX idx_local_content_records_status ON local_content_records(status);
CREATE INDEX idx_local_content_records_company_id ON local_content_records(company_id);
```
...
```sql
CREATE TABLE local_content_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES local_content_records(local_content_id),
...
CREATE INDEX CONCURRENTLY idx_local_content_records_composite
ON local_content_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_local_content_records_active
ON local_content_records(local_content_id)
...
CREATE TABLE local_content_records_y2026m01 PARTITION OF local_content_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed local content records and data
...
**Local Content Record Management**:
```typescript
// GET /api/local-content/local content records
// List local content records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/local-content/local content records
...
// PUT /api/local-content/local content records/{id}
...
const PERMISSIONS = {
  'create_local_contentRecord': ['local-content_admin', 'local-content_manager'],
  'approve_local_contentRecord': ['local-content_admin'],
  'view_local_contentRecord': ['local-content_user', 'local-content_admin', 'local-content_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Local Content Record
const local_contentRecordSchema = z.object({
  local_content_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM local_content_records
  WHERE local_content_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'local-content:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  local_contentRecord_list: ({filters}) => `local-content:local content records:list:${hash(filters)}`,
  local_contentRecord_detail: (id) => `local-content:local_contentRecord:${id}`,
  local_contentRecord_stats: () => `local-content:local content records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT local_content_id, status, status, created_at
...
-- Index Scan using idx_local_content_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Local Content Security Architecture
...
const RBAC_MATRIX = {
  'local-content_admin': {
    create: ['local content records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['local content records', 'reports'],
    approve: ['local content records', 'budgets']
  },
  'local-content_manager': {
    create: ['local content records', 'reports'],
    read: ['local content records', 'reports', 'users'],
    update: ['local content records', 'reports'],
    delete: [],
    approve: ['local content records']
  },
  'local-content_user': {
    create: ['local content records'],
    read: ['local content records', 'reports'],
    update: ['own_local content records'],
    delete: [],
    approve: []
  }
};
...
- **Local Content Creation**: active - Fully implemented with approval workflow integration
- **Local Content Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Local Content Creation
interface local_content_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'local_content_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in local_content approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Local Content Creation
...
### 4.1 Local Content Creation Modal

**Modal ID**: `LocalContentCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new local content records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 01600-2026-00001]
...
### 4.2 Local Content Details Modal

**Modal ID**: `LocalContentDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit local content record details
...
**Purpose**: Store and retrieve local content documents and attachments

**Integration Points**:
- **Document Upload**: Upload local content related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/local_content/documents/upload
/api/local_content/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for local content record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of local content status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/local_content/email/status-update
/api/local_content/email/approval-request
```
...
#### Local Content Table Component
**Component**: `Local ContentTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Local Content Record Validation
```javascript
const local_contentRecordValidationRules = {
  local_contentId: {
    required: true,
...
- **Record Not Found**: "Local Content record not found. Please check the record ID."
...
#### Lazy Loading
- Load local content records and data on demand
- Paginate large local content records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used local content records and data for 24 hours
...
- Full-text search across local content names and descriptions
- Fuzzy matching for record details
- Category-based filtering for local content records
...
#### Local Content Workflow Testing
...
- **Primary Discipline**: 01600 Local Content
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - local-content-knowledgeforge
  - local-content-knowledgeforge-local-content
...
|----------|--------|---------|----------|
| `/api/local_content/records` | GET | List local content records | Array of record objects |
| `/api/local_content/details` | GET | Get local content record details | Record detail object |

### Appendix B: Common Local Content Abbreviations
...
### Appendix C: Local Content Record Number Format

**Format**: 01600-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 01600-2026-00001 (First local content record of 2026)
- 01600-2026-01234 (1234th local content record of 2026)
...
**Container**: `A-01600-button-container`
...
- Page Load State: `local_content-baseline-page-load.png`
- Button Interactions: `local_content-baseline-button-states.png`
- Modal Layouts: `local_content-baseline-modals.png`
- Form States: `local_content-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('local_content-correspondence-btn');
