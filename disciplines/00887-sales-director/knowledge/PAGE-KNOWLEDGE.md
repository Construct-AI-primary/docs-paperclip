---
title: Sales Director Agent Architectural Knowledge
description: Comprehensive architectural knowledge of sales director system design, database schemas, API patterns, and compliance frameworks for sales director agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00887-sales-director
gigabrain_tags: knowledgeforge-ai, sales-director-agent, sales-director-architecture, system-design, compliance-frameworks
openstinger_context: sales-director-agent-architecture, sales-director-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00887_Sales Director/agent-data/domain-knowledge/00887_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00887_Sales Director/00887-sales-director-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/sales-director-knowledgeforge/SKILL.md
---
...
This document provides the Director agent with comprehensive architectural knowledge of the sales director system design, database schemas, API patterns, security frameworks, and compliance requirements. The Director agent serves as the architectural authority for sales director module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Director agent to guide sales director system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Sales Director Database Architecture
...
#### Sales Director Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE sales_director_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  sales_director_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE sales_director_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "sales_director_records_company_access" ON sales_director_records
...
CREATE INDEX idx_sales_director_records_sales_director_id ON sales_director_records(sales_director_id);
CREATE INDEX idx_sales_director_records_status ON sales_director_records(status);
CREATE INDEX idx_sales_director_records_company_id ON sales_director_records(company_id);
```
...
```sql
CREATE TABLE sales_director_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES sales_director_records(sales_director_id),
...
CREATE INDEX CONCURRENTLY idx_sales_director_records_composite
ON sales_director_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_sales_director_records_active
ON sales_director_records(sales_director_id)
...
CREATE TABLE sales_director_records_y2026m01 PARTITION OF sales_director_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed sales director records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Sales Director API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/sales-director`
...
**Versioning**: URL-based versioning (`/api/v1/sales-director`)

#### Core API Endpoints

**Sales Director Record Management**:
```typescript
// GET /api/sales-director/sales director records
// List sales director records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/sales-director/sales director records
...
// PUT /api/sales-director/sales director records/{id}
...
const PERMISSIONS = {
  'create_sales_directorRecord': ['sales-director_admin', 'sales-director_manager'],
  'approve_sales_directorRecord': ['sales-director_admin'],
  'view_sales_directorRecord': ['sales-director_user', 'sales-director_admin', 'sales-director_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Sales Director Record
const sales_directorRecordSchema = z.object({
  sales_director_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM sales_director_records
  WHERE sales_director_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'sales-director:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  sales_directorRecord_list: ({filters}) => `sales-director:sales director records:list:${hash(filters)}`,
  sales_directorRecord_detail: (id) => `sales-director:sales_directorRecord:${id}`,
  sales_directorRecord_stats: () => `sales-director:sales director records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT sales_director_id, status, status, created_at
FROM sales_director_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_sales_director_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Sales Director Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'sales-director_admin': {
    create: ['sales director records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['sales director records', 'reports'],
    approve: ['sales director records', 'budgets']
  },
  'sales-director_manager': {
    create: ['sales director records', 'reports'],
    read: ['sales director records', 'reports', 'users'],
    update: ['sales director records', 'reports'],
    delete: [],
    approve: ['sales director records']
  },
  'sales-director_user': {
    create: ['sales director records'],
    read: ['sales director records', 'reports'],
    update: ['own_sales director records'],
    delete: [],
    approve: []
  }
};
...
- **Sales Director Creation**: active - Fully implemented with approval workflow integration
- **Sales Director Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Sales Director Creation
interface sales_director_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'sales_director_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in sales_director approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Sales Director Creation
...
### 4.1 Sales Director Creation Modal

**Modal ID**: `SalesDirectorCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new sales director records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00887-2026-00001]
...
### 4.2 Sales Director Details Modal

**Modal ID**: `SalesDirectorDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit sales director record details
...
**Purpose**: Store and retrieve sales director documents and attachments

**Integration Points**:
- **Document Upload**: Upload sales director related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/sales_director/documents/upload
/api/sales_director/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for sales director record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of sales director status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/sales_director/email/status-update
/api/sales_director/email/approval-request
```
...
#### Sales Director Table Component
**Component**: `Sales DirectorTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Sales Director Record Validation
```javascript
const sales_directorRecordValidationRules = {
  sales_directorId: {
    required: true,
...
- **Record Not Found**: "Sales Director record not found. Please check the record ID."
...
#### Lazy Loading
- Load sales director records and data on demand
- Paginate large sales director records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used sales director records and data for 24 hours
...
- Full-text search across sales director names and descriptions
- Fuzzy matching for record details
- Category-based filtering for sales director records
...
#### Sales Director Workflow Testing
...
- **Primary Discipline**: 00887 Sales Director
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - sales-director-knowledgeforge
  - sales-director-knowledgeforge-sales-director
...
|----------|--------|---------|----------|
| `/api/sales_director/records` | GET | List sales director records | Array of record objects |
| `/api/sales_director/details` | GET | Get sales director record details | Record detail object |

### Appendix B: Common Sales Director Abbreviations
...
### Appendix C: Sales Director Record Number Format

**Format**: 00887-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00887-2026-00001 (First sales director record of 2026)
- 00887-2026-01234 (1234th sales director record of 2026)
...
**Container**: `A-00887-button-container`
...
- Page Load State: `sales_director-baseline-page-load.png`
- Button Interactions: `sales_director-baseline-button-states.png`
- Modal Layouts: `sales_director-baseline-modals.png`
- Form States: `sales_director-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('sales_director-correspondence-btn');
