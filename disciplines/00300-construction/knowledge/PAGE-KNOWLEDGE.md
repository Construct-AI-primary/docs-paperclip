---
title: Construction Agent Architectural Knowledge
description: Comprehensive architectural knowledge of construction system design, database schemas, API patterns, and compliance frameworks for construction agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00300-construction
gigabrain_tags: knowledgeforge-ai, construction-agent, construction-architecture, system-design, compliance-frameworks
openstinger_context: construction-agent-architecture, construction-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00300_Construction/agent-data/domain-knowledge/00300_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00300_Construction/00300-construction-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/construction-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the construction system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for construction module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide construction system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Construction Database Architecture
...
#### Construction Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE construction_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  construction_id VARCHAR(20) UNIQUE NOT NULL,
...
CREATE POLICY "construction_records_company_access" ON construction_records
...
CREATE INDEX idx_construction_records_construction_id ON construction_records(construction_id);
CREATE INDEX idx_construction_records_status ON construction_records(status);
CREATE INDEX idx_construction_records_company_id ON construction_records(company_id);
```
...
CREATE TABLE construction_details (
  ...
  value TEXT NOT NULL REFERENCES construction_records(construction_id),
...
CREATE INDEX CONCURRENTLY idx_construction_records_composite
ON construction_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_construction_records_active
ON construction_records(construction_id)
...
CREATE TABLE construction_records_y2026m01 PARTITION OF construction_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed construction records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Construction API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/construction`
...
**Versioning**: URL-based versioning (`/api/v1/construction`)

#### Core API Endpoints

**Construction Record Management**:
```typescript
// GET /api/construction/construction records
// List construction records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/construction/construction records
...
// PUT /api/construction/construction records/{id}
...
const PERMISSIONS = {
  'create_constructionRecord': ['construction_admin', 'construction_manager'],
  'approve_constructionRecord': ['construction_admin'],
  'view_constructionRecord': ['construction_user', 'construction_admin', 'construction_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Construction Record
const constructionRecordSchema = z.object({
  construction_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  ...
  WHERE construction_id = ${userInput}
...
// Cache Keys Pattern
const CACHE_KEYS = {
  constructionRecord_list: ({filters}) => `construction:construction records:list:${hash(filters)}`,
  constructionRecord_detail: (id) => `construction:constructionRecord:${id}`,
  constructionRecord_stats: () => `construction:construction records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT construction_id, status, status, created_at
...
-- Index Scan using idx_construction_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Construction Security Architecture
...
const RBAC_MATRIX = {
  'construction_admin': {
    create: ['construction records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['construction records', 'reports'],
    approve: ['construction records', 'budgets']
  },
  'construction_manager': {
    create: ['construction records', 'reports'],
    read: ['construction records', 'reports', 'users'],
    update: ['construction records', 'reports'],
    delete: [],
    approve: ['construction records']
  },
  'construction_user': {
    create: ['construction records'],
    read: ['construction records', 'reports'],
    update: ['own_construction records'],
    delete: [],
    approve: []
  }
};
...
- **Construction Creation**: active - Fully implemented with approval workflow integration
- **Construction Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Construction Creation
interface construction_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'construction_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in construction approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Construction Creation
...
### 4.1 Construction Creation Modal

**Modal ID**: `ConstructionCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new construction records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00300-2026-00001]
...
### 4.2 Construction Details Modal

**Modal ID**: `ConstructionDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit construction record details
...
**Purpose**: Store and retrieve construction documents and attachments

**Integration Points**:
- **Document Upload**: Upload construction related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/construction/documents/upload
/api/construction/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for construction record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of construction status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/construction/email/status-update
/api/construction/email/approval-request
```
...
```javascript
const constructionRecordValidationRules = {
  constructionId: {
    required: true,
...
- **Record Not Found**: "Construction record not found. Please check the record ID."
...
#### Lazy Loading
- Load construction records and data on demand
- Paginate large construction records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used construction records and data for 24 hours
...
- Full-text search across construction names and descriptions
- Fuzzy matching for record details
- Category-based filtering for construction records
...
- **Primary Discipline**: 00300 Construction
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - construction-knowledgeforge
  - construction-knowledgeforge-construction
...
|----------|--------|---------|----------|
| `/api/construction/records` | GET | List construction records | Array of record objects |
| `/api/construction/details` | GET | Get construction record details | Record detail object |

### Appendix B: Common Construction Abbreviations
...
### Appendix C: Construction Record Number Format

**Format**: 00300-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00300-2026-00001 (First construction record of 2026)
- 00300-2026-01234 (1234th construction record of 2026)
...
- Page Load State: `construction-baseline-page-load.png`
- Button Interactions: `construction-baseline-button-states.png`
- Modal Layouts: `construction-baseline-modals.png`
- Form States: `construction-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('construction-correspondence-btn');
