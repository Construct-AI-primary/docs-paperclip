---
title: Agent Creator Agent Architectural Knowledge
description: Comprehensive architectural knowledge of agent creator system design, database schemas, API patterns, and compliance frameworks for agent creator agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/02070-agent-creator
gigabrain_tags: knowledgeforge-ai, agent-creator-agent, agent-creator-architecture, system-design, compliance-frameworks
openstinger_context: agent-creator-agent-architecture, agent-creator-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/02070_Agent Creator/agent-data/domain-knowledge/02070_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/02070_Agent Creator/02070-agent-creator-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/agent-creator-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the agent creator system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for agent creator module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide agent creator system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Agent Creator Database Architecture
...
#### Agent Creator Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE agent_creator_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  agent_creator_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE agent_creator_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "agent_creator_records_company_access" ON agent_creator_records
...
CREATE INDEX idx_agent_creator_records_agent_creator_id ON agent_creator_records(agent_creator_id);
CREATE INDEX idx_agent_creator_records_status ON agent_creator_records(status);
CREATE INDEX idx_agent_creator_records_company_id ON agent_creator_records(company_id);
```
...
```sql
CREATE TABLE agent_creator_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES agent_creator_records(agent_creator_id),
...
CREATE INDEX CONCURRENTLY idx_agent_creator_records_composite
ON agent_creator_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_agent_creator_records_active
ON agent_creator_records(agent_creator_id)
...
CREATE TABLE agent_creator_records_y2026m01 PARTITION OF agent_creator_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed agent creator records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Agent Creator API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/agent-creator`
...
**Versioning**: URL-based versioning (`/api/v1/agent-creator`)

#### Core API Endpoints

**Agent Creator Record Management**:
```typescript
// GET /api/agent-creator/agent creator records
// List agent creator records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/agent-creator/agent creator records
...
// PUT /api/agent-creator/agent creator records/{id}
...
const PERMISSIONS = {
  'create_agent_creatorRecord': ['agent-creator_admin', 'agent-creator_manager'],
  'approve_agent_creatorRecord': ['agent-creator_admin'],
  'view_agent_creatorRecord': ['agent-creator_user', 'agent-creator_admin', 'agent-creator_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Agent Creator Record
const agent_creatorRecordSchema = z.object({
  agent_creator_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM agent_creator_records
  WHERE agent_creator_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'agent-creator:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  agent_creatorRecord_list: ({filters}) => `agent-creator:agent creator records:list:${hash(filters)}`,
  agent_creatorRecord_detail: (id) => `agent-creator:agent_creatorRecord:${id}`,
  agent_creatorRecord_stats: () => `agent-creator:agent creator records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT agent_creator_id, status, status, created_at
FROM agent_creator_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_agent_creator_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Agent Creator Security Architecture
...
const RBAC_MATRIX = {
  'agent-creator_admin': {
    create: ['agent creator records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['agent creator records', 'reports'],
    approve: ['agent creator records', 'budgets']
  },
  'agent-creator_manager': {
    create: ['agent creator records', 'reports'],
    read: ['agent creator records', 'reports', 'users'],
    update: ['agent creator records', 'reports'],
    delete: [],
    approve: ['agent creator records']
  },
  'agent-creator_user': {
    create: ['agent creator records'],
    read: ['agent creator records', 'reports'],
    update: ['own_agent creator records'],
    delete: [],
    approve: []
  }
};
...
- **Agent Creator Creation**: active - Fully implemented with approval workflow integration
- **Agent Creator Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Agent Creator Creation
interface agent_creator_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'agent_creator_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in agent_creator approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Agent Creator Creation
...
### 4.1 Agent Creator Creation Modal

**Modal ID**: `AgentCreatorCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new agent creator records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 02070-2026-00001]
...
### 4.2 Agent Creator Details Modal

**Modal ID**: `AgentCreatorDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit agent creator record details
...
**Purpose**: Store and retrieve agent creator documents and attachments

**Integration Points**:
- **Document Upload**: Upload agent creator related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/agent_creator/documents/upload
/api/agent_creator/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for agent creator record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of agent creator status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/agent_creator/email/status-update
/api/agent_creator/email/approval-request
```
...
#### Agent Creator Table Component
**Component**: `Agent CreatorTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Agent Creator Record Validation
```javascript
const agent_creatorRecordValidationRules = {
  agent_creatorId: {
    required: true,
...
- **Record Not Found**: "Agent Creator record not found. Please check the record ID."
...
#### Lazy Loading
- Load agent creator records and data on demand
- Paginate large agent creator records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used agent creator records and data for 24 hours
...
- Full-text search across agent creator names and descriptions
- Fuzzy matching for record details
- Category-based filtering for agent creator records
...
#### Agent Creator Workflow Testing
...
- **Primary Discipline**: 02070 Agent Creator
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - agent-creator-knowledgeforge
  - agent-creator-knowledgeforge-agent-creator
...
|----------|--------|---------|----------|
| `/api/agent_creator/records` | GET | List agent creator records | Array of record objects |
| `/api/agent_creator/details` | GET | Get agent creator record details | Record detail object |

### Appendix B: Common Agent Creator Abbreviations
...
### Appendix C: Agent Creator Record Number Format

**Format**: 02070-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 02070-2026-00001 (First agent creator record of 2026)
- 02070-2026-01234 (1234th agent creator record of 2026)
...
**Container**: `A-02070-button-container`
...
- Page Load State: `agent_creator-baseline-page-load.png`
- Button Interactions: `agent_creator-baseline-button-states.png`
- Modal Layouts: `agent_creator-baseline-modals.png`
- Form States: `agent_creator-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('agent_creator-correspondence-btn');
