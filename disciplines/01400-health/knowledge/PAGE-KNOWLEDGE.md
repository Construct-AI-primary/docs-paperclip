---
title: Health Agent Architectural Knowledge
description: Comprehensive architectural knowledge of health system design, database schemas, API patterns, and compliance frameworks for health agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/01400-health
gigabrain_tags: knowledgeforge-ai, health-agent, health-architecture, system-design, compliance-frameworks
openstinger_context: health-agent-architecture, health-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/01400_Health/agent-data/domain-knowledge/01400_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/01400_Health/01400-health-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/health-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the health system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for health module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide health system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Health Database Architecture
...
#### Health Records Table Architecture
```sql
...
CREATE POLICY "health_records_company_access" ON health_records
...
CREATE INDEX idx_health_records_health_id ON health_records(health_id);
CREATE INDEX idx_health_records_status ON health_records(status);
CREATE INDEX idx_health_records_company_id ON health_records(company_id);
```
...
CREATE TABLE health_details (
  ...
  value TEXT NOT NULL REFERENCES health_records(health_id),
...
CREATE INDEX CONCURRENTLY idx_health_records_composite
ON health_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_health_records_active
ON health_records(health_id)
...
CREATE TABLE health_records_y2026m01 PARTITION OF health_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed health records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Health API Architecture
...
**Versioning**: URL-based versioning (`/api/v1/health`)
...
// List health records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
const PERMISSIONS = {
  'create_healthRecord': ['health_admin', 'health_manager'],
  'approve_healthRecord': ['health_admin'],
  'view_healthRecord': ['health_user', 'health_admin', 'health_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Health Record
const healthRecordSchema = z.object({
  health_id: z.string().min(1).max(50),
...
// Cache Keys Pattern
const CACHE_KEYS = {
  healthRecord_list: ({filters}) => `health:health records:list:${hash(filters)}`,
  healthRecord_detail: (id) => `health:healthRecord:${id}`,
  healthRecord_stats: () => `health:health records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT health_id, status, status, created_at
FROM health_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_health_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Health Security Architecture
...
const RBAC_MATRIX = {
  'health_admin': {
    create: ['health records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['health records', 'reports'],
    approve: ['health records', 'budgets']
  },
  'health_manager': {
    create: ['health records', 'reports'],
    read: ['health records', 'reports', 'users'],
    update: ['health records', 'reports'],
    delete: [],
    approve: ['health records']
  },
  'health_user': {
    create: ['health records'],
    read: ['health records', 'reports'],
    update: ['own_health records'],
    delete: [],
    approve: []
  }
};
...
- **Health Creation**: active - Fully implemented with approval workflow integration
- **Health Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Health Creation
interface health_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'health_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in health approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Health Creation
...
### 4.1 Health Creation Modal

**Modal ID**: `HealthCreationModal`
...
1. **Basic Information**
   ```
   Record ID: [Auto-generated: 01400-2026-00001]
...
### 4.2 Health Details Modal

**Modal ID**: `HealthDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit health record details
...
**Purpose**: Store and retrieve health documents and attachments

**Integration Points**:
- **Document Upload**: Upload health related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/health/documents/upload
/api/health/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for health record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of health status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/health/email/status-update
/api/health/email/approval-request
```
...
- **Record Not Found**: "Health record not found. Please check the record ID."
...
#### Lazy Loading
- Load health records and data on demand
- Paginate large health records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used health records and data for 24 hours
...
- Full-text search across health names and descriptions
- Fuzzy matching for record details
- Category-based filtering for health records
...
- **Primary Discipline**: 01400 Health
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - health-knowledgeforge
  - health-knowledgeforge-health
...
|----------|--------|---------|----------|
| `/api/health/records` | GET | List health records | Array of record objects |
| `/api/health/details` | GET | Get health record details | Record detail object |

### Appendix B: Common Health Abbreviations
...
### Appendix C: Health Record Number Format

**Format**: 01400-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 01400-2026-00001 (First health record of 2026)
- 01400-2026-01234 (1234th health record of 2026)
...
- Page Load State: `health-baseline-page-load.png`
- Button Interactions: `health-baseline-button-states.png`
- Modal Layouts: `health-baseline-modals.png`
- Form States: `health-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('health-correspondence-btn');
