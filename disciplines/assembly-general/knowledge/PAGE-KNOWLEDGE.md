---
title: General Agent Architectural Knowledge
description: Comprehensive architectural knowledge of general system design, database schemas, API patterns, and compliance frameworks for general agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/assembly-general
gigabrain_tags: knowledgeforge-ai, specialist-asse-agent, general-architecture, system-design, compliance-frameworks
openstinger_context: general-agent-architecture, specialist-asse-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/assembly_General/agent-data/domain-knowledge/assembly_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/assembly_General/assembly-general-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/specialist-asse-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the general system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for general module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide general system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: General Database Architecture
...
#### General Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE _records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  _id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE _records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "_records_company_access" ON _records
...
CREATE INDEX idx__records__id ON _records(_id);
CREATE INDEX idx__records_status ON _records(status);
CREATE INDEX idx__records_company_id ON _records(company_id);
```
...
CREATE TABLE _details (
  ...
  value TEXT NOT NULL REFERENCES _records(_id),
...
CREATE INDEX CONCURRENTLY idx__records_composite
ON _records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx__records_active
ON _records(_id)
...
CREATE TABLE _records_y2026m01 PARTITION OF _records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed  records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: General API Architecture
...
**Versioning**: URL-based versioning (`/api/v1/general`)

#### Core API Endpoints

**General Record Management**:
```typescript
// GET /api/general/ records
...
const PERMISSIONS = {
  'create_Record': ['specialist-asse_admin', 'specialist-asse_manager'],
  'approve_Record': ['specialist-asse_admin'],
  'view_Record': ['specialist-asse_user', 'specialist-asse_admin', 'specialist-asse_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for General Record
...
// Cache Keys Pattern
const CACHE_KEYS = {
  Record_list: ({filters}) => `general: records:list:${hash(filters)}`,
  Record_detail: (id) => `general:Record:${id}`,
  Record_stats: () => `general: records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT _id, status, status, created_at
FROM _records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx__records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: General Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'specialist-asse_admin': {
    create: [' records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: [' records', 'reports'],
    approve: [' records', 'budgets']
  },
  'specialist-asse_manager': {
    create: [' records', 'reports'],
    read: [' records', 'reports', 'users'],
    update: [' records', 'reports'],
    delete: [],
    approve: [' records']
  },
  'specialist-asse_user': {
    create: [' records'],
    read: [' records', 'reports'],
    update: ['own_ records'],
    delete: [],
    approve: []
  }
...
- **General Creation**: active - Fully implemented with approval workflow integration
- **General Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of General Creation
interface _creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  '_creation_workflow': {
    ...
    knownIssues: ['ISSUE-001: Race condition in  approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for General Creation
...
### 4.1 General Creation Modal

**Modal ID**: `GeneralCreationModal`
...
1. **Basic Information**
   ```
   Record ID: [Auto-generated: ASSEMBLY-2026-00001]
...
### 4.2 General Details Modal

**Modal ID**: `GeneralDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit  record details
...
**Purpose**: Store and retrieve  documents and attachments

**Integration Points**:
- **Document Upload**: Upload  related documents
...
**Purpose**: Send notifications for  record updates
...
- **Record Not Found**: "General record not found. Please check the record ID."
...
#### Lazy Loading
- Load  records and data on demand
- Paginate large  records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used  records and data for 24 hours
...
- Category-based filtering for  records
...
- **Primary Discipline**: assembly General
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - specialist-asse-knowledgeforge
  - specialist-asse-knowledgeforge-general
...
|----------|--------|---------|----------|
| `/api//records` | GET | List  records | Array of record objects |
| `/api//details` | GET | Get  record details | Record detail object |

### Appendix B: Common General Abbreviations
...
### Appendix C: General Record Number Format

**Format**: ASSEMBLY-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- ASSEMBLY-2026-00001 (First  record of 2026)
- ASSEMBLY-2026-01234 (1234th  record of 2026)
...
**Container**: `A-assembly-button-container`
...
- Page Load State: `-baseline-page-load.png`
- Button Interactions: `-baseline-button-states.png`
- Modal Layouts: `-baseline-modals.png`
- Form States: `-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('-correspondence-btn');
