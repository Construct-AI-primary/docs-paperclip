...
title: Implement Hazard Identification and Risk Assessment Mobile Offline Synchronization
...
# SAFETY-HAZARD-002: Implement Hazard Identification and Risk Assessment Mobile Offline Synchronization

## Description

Implement offline data synchronization for the Hazard Identification and Risk Assessment mobile application. Construction sites frequently lack reliable connectivity, so the mobile app must function fully offline and sync when connectivity returns.

**Context**: Construction sites often have poor or no cellular/WiFi connectivity. Field workers must be able to create, edit, and review hazard identification and risk assessment records without network access, with seamless synchronization when connectivity is restored.
...
- **Target Files**:
  - `packages/shared/src/safety/safetyhazard/offline-store.ts`
  - `packages/shared/src/safety/safetyhazard/sync-engine.ts`
