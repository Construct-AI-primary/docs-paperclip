...
title: Implement Emergency Preparedness and Response Mobile Offline Synchronization
...
# SAFETY-EMERGENCY-002: Implement Emergency Preparedness and Response Mobile Offline Synchronization

## Description

Implement offline data synchronization for the Emergency Preparedness and Response mobile application. Construction sites frequently lack reliable connectivity, so the mobile app must function fully offline and sync when connectivity returns.

**Context**: Construction sites often have poor or no cellular/WiFi connectivity. Field workers must be able to create, edit, and review emergency preparedness and response records without network access, with seamless synchronization when connectivity is restored.
...
- **Target Files**:
  - `packages/shared/src/safety/safetyemergency/offline-store.ts`
  - `packages/shared/src/safety/safetyemergency/sync-engine.ts`
