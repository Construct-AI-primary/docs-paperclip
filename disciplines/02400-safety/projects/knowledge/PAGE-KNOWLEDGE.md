---
title: Safety Agent Architectural Knowledge
description: Comprehensive architectural knowledge of safety system design, database schemas, API patterns, and compliance frameworks for safety agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/02400-safety
gigabrain_tags: knowledgeforge-ai, safety-agent, safety-architecture, system-design, compliance-frameworks
openstinger_context: safety-agent-architecture, safety-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/02400_Safety/agent-data/domain-knowledge/02400_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/02400_Safety/02400-safety-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/safety-knowledgeforge/SKILL.md
---
...
This document provides the Inspector agent with comprehensive architectural knowledge of the safety system design, database schemas, API patterns, security frameworks, and compliance requirements. The Inspector agent serves as the architectural authority for safety module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Inspector agent to guide safety system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Safety Database Architecture
...
#### Safety Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE safety_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  safety_id VARCHAR(20) UNIQUE NOT NULL,
...
CREATE POLICY "safety_records_company_access" ON safety_records
...
CREATE INDEX idx_safety_records_safety_id ON safety_records(safety_id);
CREATE INDEX idx_safety_records_status ON safety_records(status);
CREATE INDEX idx_safety_records_company_id ON safety_records(company_id);
```
...
CREATE TABLE safety_details (
  ...
  value TEXT NOT NULL REFERENCES safety_records(safety_id),
...
CREATE INDEX CONCURRENTLY idx_safety_records_composite
ON safety_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_safety_records_active
ON safety_records(safety_id)
...
CREATE TABLE safety_records_y2026m01 PARTITION OF safety_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed safety records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Safety API Architecture
...
**Versioning**: URL-based versioning (`/api/v1/safety`)
...
// List safety records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/safety/safety records
...
// PUT /api/safety/safety records/{id}
...
const PERMISSIONS = {
  'create_safetyRecord': ['safety_admin', 'safety_manager'],
  'approve_safetyRecord': ['safety_admin'],
  'view_safetyRecord': ['safety_user', 'safety_admin', 'safety_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Safety Record
const safetyRecordSchema = z.object({
  safety_id: z.string().min(1).max(50),
...
// Cache Keys Pattern
const CACHE_KEYS = {
  safetyRecord_list: ({filters}) => `safety:safety records:list:${hash(filters)}`,
  safetyRecord_detail: (id) => `safety:safetyRecord:${id}`,
  safetyRecord_stats: () => `safety:safety records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT safety_id, status, status, created_at
FROM safety_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_safety_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Safety Security Architecture
...
const RBAC_MATRIX = {
  'safety_admin': {
    create: ['safety records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['safety records', 'reports'],
    approve: ['safety records', 'budgets']
  },
  'safety_manager': {
    create: ['safety records', 'reports'],
    read: ['safety records', 'reports', 'users'],
    update: ['safety records', 'reports'],
    delete: [],
    approve: ['safety records']
  },
  'safety_user': {
    create: ['safety records'],
    read: ['safety records', 'reports'],
    update: ['own_safety records'],
    delete: [],
    approve: []
  }
};
...
- **Safety Creation**: active - Fully implemented with approval workflow integration
- **Safety Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Safety Creation
interface safety_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'safety_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in safety approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Safety Creation
...
### 4.1 Safety Creation Modal

**Modal ID**: `SafetyCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new safety records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 02400-2026-00001]
...
### 4.2 Safety Details Modal

**Modal ID**: `SafetyDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit safety record details
...
**Purpose**: Store and retrieve safety documents and attachments

**Integration Points**:
- **Document Upload**: Upload safety related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/safety/documents/upload
/api/safety/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for safety record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of safety status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/safety/email/status-update
/api/safety/email/approval-request
```
...
#### Safety Table Component
**Component**: `SafetyTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
```javascript
const safetyRecordValidationRules = {
  safetyId: {
    required: true,
...
- **Record Not Found**: "Safety record not found. Please check the record ID."
...
#### Lazy Loading
- Load safety records and data on demand
- Paginate large safety records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used safety records and data for 24 hours
...
- Full-text search across safety names and descriptions
- Fuzzy matching for record details
- Category-based filtering for safety records
...
- **Primary Discipline**: 02400 Safety
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - safety-knowledgeforge
  - safety-knowledgeforge-safety
...
|----------|--------|---------|----------|
| `/api/safety/records` | GET | List safety records | Array of record objects |
| `/api/safety/details` | GET | Get safety record details | Record detail object |

### Appendix B: Common Safety Abbreviations
...
### Appendix C: Safety Record Number Format

**Format**: 02400-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 02400-2026-00001 (First safety record of 2026)
- 02400-2026-01234 (1234th safety record of 2026)
...
**Container**: `A-02400-button-container`
...
- Page Load State: `safety-baseline-page-load.png`
- Button Interactions: `safety-baseline-button-states.png`
- Modal Layouts: `safety-baseline-modals.png`
- Form States: `safety-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('safety-correspondence-btn');
