---
title: Director Project Agent Architectural Knowledge
description: Comprehensive architectural knowledge of director project system design, database schemas, API patterns, and compliance frameworks for director project agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00895-director-project
gigabrain_tags: knowledgeforge-ai, director-project-agent, director-project-architecture, system-design, compliance-frameworks
openstinger_context: director-project-agent-architecture, director-project-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00895_Director Project/agent-data/domain-knowledge/00895_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00895_Director Project/00895-director-project-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/director-project-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the director project system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for director project module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide director project system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Director Project Database Architecture
...
#### Director Project Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE director_project_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  director_project_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE director_project_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "director_project_records_company_access" ON director_project_records
...
CREATE INDEX idx_director_project_records_director_project_id ON director_project_records(director_project_id);
CREATE INDEX idx_director_project_records_status ON director_project_records(status);
CREATE INDEX idx_director_project_records_company_id ON director_project_records(company_id);
```
...
```sql
CREATE TABLE director_project_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES director_project_records(director_project_id),
...
CREATE INDEX CONCURRENTLY idx_director_project_records_composite
ON director_project_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_director_project_records_active
ON director_project_records(director_project_id)
...
CREATE TABLE director_project_records_y2026m01 PARTITION OF director_project_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed director project records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Director Project API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/director-project`
...
**Versioning**: URL-based versioning (`/api/v1/director-project`)

#### Core API Endpoints

**Director Project Record Management**:
```typescript
// GET /api/director-project/director project records
// List director project records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/director-project/director project records
...
// PUT /api/director-project/director project records/{id}
...
const PERMISSIONS = {
  'create_director_projectRecord': ['director-project_admin', 'director-project_manager'],
  'approve_director_projectRecord': ['director-project_admin'],
  'view_director_projectRecord': ['director-project_user', 'director-project_admin', 'director-project_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Director Project Record
const director_projectRecordSchema = z.object({
  director_project_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM director_project_records
  WHERE director_project_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'director-project:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  director_projectRecord_list: ({filters}) => `director-project:director project records:list:${hash(filters)}`,
  director_projectRecord_detail: (id) => `director-project:director_projectRecord:${id}`,
  director_projectRecord_stats: () => `director-project:director project records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT director_project_id, status, status, created_at
FROM director_project_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_director_project_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Director Project Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'director-project_admin': {
    create: ['director project records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['director project records', 'reports'],
    approve: ['director project records', 'budgets']
  },
  'director-project_manager': {
    create: ['director project records', 'reports'],
    read: ['director project records', 'reports', 'users'],
    update: ['director project records', 'reports'],
    delete: [],
    approve: ['director project records']
  },
  'director-project_user': {
    create: ['director project records'],
    read: ['director project records', 'reports'],
    update: ['own_director project records'],
    delete: [],
    approve: []
  }
};
...
- **Director Project Creation**: active - Fully implemented with approval workflow integration
- **Director Project Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Director Project Creation
interface director_project_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'director_project_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in director_project approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Director Project Creation
...
### 4.1 Director Project Creation Modal

**Modal ID**: `DirectorProjectCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new director project records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00895-2026-00001]
...
### 4.2 Director Project Details Modal

**Modal ID**: `DirectorProjectDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit director project record details
...
**Purpose**: Store and retrieve director project documents and attachments

**Integration Points**:
- **Document Upload**: Upload director project related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/director_project/documents/upload
/api/director_project/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for director project record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of director project status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/director_project/email/status-update
/api/director_project/email/approval-request
```
...
#### Director Project Table Component
**Component**: `Director ProjectTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Director Project Record Validation
```javascript
const director_projectRecordValidationRules = {
  director_projectId: {
    required: true,
...
- **Record Not Found**: "Director Project record not found. Please check the record ID."
...
#### Lazy Loading
- Load director project records and data on demand
- Paginate large director project records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used director project records and data for 24 hours
...
- Full-text search across director project names and descriptions
- Fuzzy matching for record details
- Category-based filtering for director project records
...
#### Director Project Workflow Testing
...
- **Primary Discipline**: 00895 Director Project
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - director-project-knowledgeforge
  - director-project-knowledgeforge-director-project
...
|----------|--------|---------|----------|
| `/api/director_project/records` | GET | List director project records | Array of record objects |
| `/api/director_project/details` | GET | Get director project record details | Record detail object |

### Appendix B: Common Director Project Abbreviations

| Abbreviation | Full Term |
|-------------|-----------|
| 0089 | Director Project |
| REC | Record |

### Appendix C: Director Project Record Number Format

**Format**: 00895-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00895-2026-00001 (First director project record of 2026)
- 00895-2026-01234 (1234th director project record of 2026)
...
**Container**: `A-00895-button-container`
...
- Page Load State: `director_project-baseline-page-load.png`
- Button Interactions: `director_project-baseline-button-states.png`
- Modal Layouts: `director_project-baseline-modals.png`
- Form States: `director_project-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('director_project-correspondence-btn');
