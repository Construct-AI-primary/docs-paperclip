---
title: Hse Director Agent Architectural Knowledge
description: Comprehensive architectural knowledge of hse director system design, database schemas, API patterns, and compliance frameworks for hse director agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00885-hse-director
gigabrain_tags: knowledgeforge-ai, hse-director-agent, hse-director-architecture, system-design, compliance-frameworks
openstinger_context: hse-director-agent-architecture, hse-director-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00885_Hse Director/agent-data/domain-knowledge/00885_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00885_Hse Director/00885-hse-director-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/hse-director-knowledgeforge/SKILL.md
---
...
This document provides the Director agent with comprehensive architectural knowledge of the hse director system design, database schemas, API patterns, security frameworks, and compliance requirements. The Director agent serves as the architectural authority for hse director module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Director agent to guide hse director system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Hse Director Database Architecture
...
#### Hse Director Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE hse_director_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  hse_director_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE hse_director_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "hse_director_records_company_access" ON hse_director_records
...
CREATE INDEX idx_hse_director_records_hse_director_id ON hse_director_records(hse_director_id);
CREATE INDEX idx_hse_director_records_status ON hse_director_records(status);
CREATE INDEX idx_hse_director_records_company_id ON hse_director_records(company_id);
```
...
```sql
CREATE TABLE hse_director_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES hse_director_records(hse_director_id),
...
CREATE INDEX CONCURRENTLY idx_hse_director_records_composite
ON hse_director_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_hse_director_records_active
ON hse_director_records(hse_director_id)
...
CREATE TABLE hse_director_records_y2026m01 PARTITION OF hse_director_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed hse director records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Hse Director API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/hse-director`
...
**Versioning**: URL-based versioning (`/api/v1/hse-director`)

#### Core API Endpoints

**Hse Director Record Management**:
```typescript
// GET /api/hse-director/hse director records
// List hse director records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/hse-director/hse director records
...
// PUT /api/hse-director/hse director records/{id}
...
const PERMISSIONS = {
  'create_hse_directorRecord': ['hse-director_admin', 'hse-director_manager'],
  'approve_hse_directorRecord': ['hse-director_admin'],
  'view_hse_directorRecord': ['hse-director_user', 'hse-director_admin', 'hse-director_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Hse Director Record
const hse_directorRecordSchema = z.object({
  hse_director_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM hse_director_records
  WHERE hse_director_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'hse-director:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  hse_directorRecord_list: ({filters}) => `hse-director:hse director records:list:${hash(filters)}`,
  hse_directorRecord_detail: (id) => `hse-director:hse_directorRecord:${id}`,
  hse_directorRecord_stats: () => `hse-director:hse director records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT hse_director_id, status, status, created_at
FROM hse_director_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_hse_director_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Hse Director Security Architecture
...
const RBAC_MATRIX = {
  'hse-director_admin': {
    create: ['hse director records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['hse director records', 'reports'],
    approve: ['hse director records', 'budgets']
  },
  'hse-director_manager': {
    create: ['hse director records', 'reports'],
    read: ['hse director records', 'reports', 'users'],
    update: ['hse director records', 'reports'],
    delete: [],
    approve: ['hse director records']
  },
  'hse-director_user': {
    create: ['hse director records'],
    read: ['hse director records', 'reports'],
    update: ['own_hse director records'],
    delete: [],
    approve: []
  }
};
...
- **Hse Director Creation**: active - Fully implemented with approval workflow integration
- **Hse Director Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Hse Director Creation
interface hse_director_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'hse_director_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in hse_director approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Hse Director Creation
...
### 4.1 Hse Director Creation Modal

**Modal ID**: `HseDirectorCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new hse director records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00885-2026-00001]
...
### 4.2 Hse Director Details Modal

**Modal ID**: `HseDirectorDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit hse director record details
...
**Purpose**: Store and retrieve hse director documents and attachments

**Integration Points**:
- **Document Upload**: Upload hse director related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/hse_director/documents/upload
/api/hse_director/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for hse director record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of hse director status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/hse_director/email/status-update
/api/hse_director/email/approval-request
```
...
#### Hse Director Table Component
**Component**: `Hse DirectorTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Hse Director Record Validation
```javascript
const hse_directorRecordValidationRules = {
  hse_directorId: {
    required: true,
...
- **Record Not Found**: "Hse Director record not found. Please check the record ID."
...
#### Lazy Loading
- Load hse director records and data on demand
- Paginate large hse director records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used hse director records and data for 24 hours
...
- Full-text search across hse director names and descriptions
- Fuzzy matching for record details
- Category-based filtering for hse director records
...
- **Primary Discipline**: 00885 Hse Director
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - hse-director-knowledgeforge
  - hse-director-knowledgeforge-hse-director
...
|----------|--------|---------|----------|
| `/api/hse_director/records` | GET | List hse director records | Array of record objects |
| `/api/hse_director/details` | GET | Get hse director record details | Record detail object |

### Appendix B: Common Hse Director Abbreviations
...
### Appendix C: Hse Director Record Number Format

**Format**: 00885-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00885-2026-00001 (First hse director record of 2026)
- 00885-2026-01234 (1234th hse director record of 2026)
...
**Container**: `A-00885-button-container`
...
- Page Load State: `hse_director-baseline-page-load.png`
- Button Interactions: `hse_director-baseline-button-states.png`
- Modal Layouts: `hse_director-baseline-modals.png`
- Form States: `hse_director-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('hse_director-correspondence-btn');
