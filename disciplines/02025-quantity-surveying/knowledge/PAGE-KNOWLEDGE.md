---
title: Quantity Surveying Agent Architectural Knowledge
description: Comprehensive architectural knowledge of quantity surveying system design, database schemas, API patterns, and compliance frameworks for quantity surveying agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/02025-quantity-surveying
gigabrain_tags: knowledgeforge-ai, quantity-surveying-agent, quantity-surveying-architecture, system-design, compliance-frameworks
openstinger_context: quantity-surveying-agent-architecture, quantity-surveying-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/02025_Quantity Surveying/agent-data/domain-knowledge/02025_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/02025_Quantity Surveying/02025-quantity-surveying-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/quantity-surveying-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the quantity surveying system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for quantity surveying module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide quantity surveying system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Quantity Surveying Database Architecture
...
#### Quantity Surveying Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE quantity_surveying_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  quantity_surveying_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE quantity_surveying_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "quantity_surveying_records_company_access" ON quantity_surveying_records
...
CREATE INDEX idx_quantity_surveying_records_quantity_surveying_id ON quantity_surveying_records(quantity_surveying_id);
CREATE INDEX idx_quantity_surveying_records_status ON quantity_surveying_records(status);
CREATE INDEX idx_quantity_surveying_records_company_id ON quantity_surveying_records(company_id);
```
...
```sql
CREATE TABLE quantity_surveying_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES quantity_surveying_records(quantity_surveying_id),
...
CREATE INDEX CONCURRENTLY idx_quantity_surveying_records_composite
ON quantity_surveying_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_quantity_surveying_records_active
ON quantity_surveying_records(quantity_surveying_id)
...
CREATE TABLE quantity_surveying_records_y2026m01 PARTITION OF quantity_surveying_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed quantity surveying records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Quantity Surveying API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/quantity-surveying`
...
**Versioning**: URL-based versioning (`/api/v1/quantity-surveying`)

#### Core API Endpoints

**Quantity Surveying Record Management**:
```typescript
// GET /api/quantity-surveying/quantity surveying records
// List quantity surveying records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/quantity-surveying/quantity surveying records
...
// PUT /api/quantity-surveying/quantity surveying records/{id}
...
const PERMISSIONS = {
  'create_quantity_surveyingRecord': ['quantity-surveying_admin', 'quantity-surveying_manager'],
  'approve_quantity_surveyingRecord': ['quantity-surveying_admin'],
  'view_quantity_surveyingRecord': ['quantity-surveying_user', 'quantity-surveying_admin', 'quantity-surveying_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Quantity Surveying Record
const quantity_surveyingRecordSchema = z.object({
  quantity_surveying_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM quantity_surveying_records
  WHERE quantity_surveying_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'quantity-surveying:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  quantity_surveyingRecord_list: ({filters}) => `quantity-surveying:quantity surveying records:list:${hash(filters)}`,
  quantity_surveyingRecord_detail: (id) => `quantity-surveying:quantity_surveyingRecord:${id}`,
  quantity_surveyingRecord_stats: () => `quantity-surveying:quantity surveying records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT quantity_surveying_id, status, status, created_at
FROM quantity_surveying_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_quantity_surveying_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Quantity Surveying Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'quantity-surveying_admin': {
    create: ['quantity surveying records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['quantity surveying records', 'reports'],
    approve: ['quantity surveying records', 'budgets']
  },
  'quantity-surveying_manager': {
    create: ['quantity surveying records', 'reports'],
    read: ['quantity surveying records', 'reports', 'users'],
    update: ['quantity surveying records', 'reports'],
    delete: [],
    approve: ['quantity surveying records']
  },
  'quantity-surveying_user': {
    create: ['quantity surveying records'],
    read: ['quantity surveying records', 'reports'],
    update: ['own_quantity surveying records'],
    delete: [],
    approve: []
  }
};
...
- **Quantity Surveying Creation**: active - Fully implemented with approval workflow integration
- **Quantity Surveying Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Quantity Surveying Creation
interface quantity_surveying_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'quantity_surveying_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in quantity_surveying approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Quantity Surveying Creation
...
### 4.1 Quantity Surveying Creation Modal

**Modal ID**: `QuantitySurveyingCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new quantity surveying records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 02025-2026-00001]
...
### 4.2 Quantity Surveying Details Modal

**Modal ID**: `QuantitySurveyingDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit quantity surveying record details
...
**Purpose**: Store and retrieve quantity surveying documents and attachments

**Integration Points**:
- **Document Upload**: Upload quantity surveying related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/quantity_surveying/documents/upload
/api/quantity_surveying/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for quantity surveying record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of quantity surveying status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/quantity_surveying/email/status-update
/api/quantity_surveying/email/approval-request
```
...
#### Quantity Surveying Table Component
**Component**: `Quantity SurveyingTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Quantity Surveying Record Validation
```javascript
const quantity_surveyingRecordValidationRules = {
  quantity_surveyingId: {
    required: true,
...
- **Record Not Found**: "Quantity Surveying record not found. Please check the record ID."
...
#### Lazy Loading
- Load quantity surveying records and data on demand
- Paginate large quantity surveying records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used quantity surveying records and data for 24 hours
...
- Full-text search across quantity surveying names and descriptions
- Fuzzy matching for record details
- Category-based filtering for quantity surveying records
...
#### Quantity Surveying Workflow Testing
...
- **Primary Discipline**: 02025 Quantity Surveying
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - quantity-surveying-knowledgeforge
  - quantity-surveying-knowledgeforge-quantity-surveying
...
|----------|--------|---------|----------|
| `/api/quantity_surveying/records` | GET | List quantity surveying records | Array of record objects |
| `/api/quantity_surveying/details` | GET | Get quantity surveying record details | Record detail object |

### Appendix B: Common Quantity Surveying Abbreviations

| Abbreviation | Full Term |
|-------------|-----------|
| 0202 | Quantity Surveying |
| REC | Record |

### Appendix C: Quantity Surveying Record Number Format

**Format**: 02025-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 02025-2026-00001 (First quantity surveying record of 2026)
- 02025-2026-01234 (1234th quantity surveying record of 2026)
...
- Page Load State: `quantity_surveying-baseline-page-load.png`
- Button Interactions: `quantity_surveying-baseline-button-states.png`
- Modal Layouts: `quantity_surveying-baseline-modals.png`
- Form States: `quantity_surveying-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('quantity_surveying-correspondence-btn');
