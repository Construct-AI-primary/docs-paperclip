---
title: Construction Director Agent Architectural Knowledge
description: Comprehensive architectural knowledge of construction director system design, database schemas, API patterns, and compliance frameworks for construction director agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/00882-construction-director
gigabrain_tags: knowledgeforge-ai, construction-director-agent, construction-director-architecture, system-design, compliance-frameworks
openstinger_context: construction-director-agent-architecture, construction-director-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/00882_Construction Director/agent-data/domain-knowledge/00882_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/00882_Construction Director/00882-construction-director-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/construction-director-knowledgeforge/SKILL.md
---

# Director Agent Architectural Knowledge

## Overview

This document provides the Director agent with comprehensive architectural knowledge of the construction director system design, database schemas, API patterns, security frameworks, and compliance requirements. The Director agent serves as the architectural authority for construction director module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Director agent to guide construction director system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Construction Director Database Architecture
...
#### Construction Director Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE construction_director_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  construction_director_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE construction_director_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "construction_director_records_company_access" ON construction_director_records
...
CREATE INDEX idx_construction_director_records_construction_director_id ON construction_director_records(construction_director_id);
CREATE INDEX idx_construction_director_records_status ON construction_director_records(status);
CREATE INDEX idx_construction_director_records_company_id ON construction_director_records(company_id);
```
...
```sql
CREATE TABLE construction_director_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES construction_director_records(construction_director_id),
...
CREATE INDEX CONCURRENTLY idx_construction_director_records_composite
ON construction_director_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_construction_director_records_active
ON construction_director_records(construction_director_id)
...
CREATE TABLE construction_director_records_y2026m01 PARTITION OF construction_director_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed construction director records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Construction Director API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/construction-director`
...
**Versioning**: URL-based versioning (`/api/v1/construction-director`)

#### Core API Endpoints

**Construction Director Record Management**:
```typescript
// GET /api/construction-director/construction director records
// List construction director records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/construction-director/construction director records
...
// PUT /api/construction-director/construction director records/{id}
...
const PERMISSIONS = {
  'create_construction_directorRecord': ['construction-director_admin', 'construction-director_manager'],
  'approve_construction_directorRecord': ['construction-director_admin'],
  'view_construction_directorRecord': ['construction-director_user', 'construction-director_admin', 'construction-director_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Construction Director Record
const construction_directorRecordSchema = z.object({
  construction_director_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM construction_director_records
  WHERE construction_director_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'construction-director:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  construction_directorRecord_list: ({filters}) => `construction-director:construction director records:list:${hash(filters)}`,
  construction_directorRecord_detail: (id) => `construction-director:construction_directorRecord:${id}`,
  construction_directorRecord_stats: () => `construction-director:construction director records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT construction_director_id, status, status, created_at
FROM construction_director_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_construction_director_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Construction Director Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'construction-director_admin': {
    create: ['construction director records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['construction director records', 'reports'],
    approve: ['construction director records', 'budgets']
  },
  'construction-director_manager': {
    create: ['construction director records', 'reports'],
    read: ['construction director records', 'reports', 'users'],
    update: ['construction director records', 'reports'],
    delete: [],
    approve: ['construction director records']
  },
  'construction-director_user': {
    create: ['construction director records'],
    read: ['construction director records', 'reports'],
    update: ['own_construction director records'],
    delete: [],
    approve: []
  }
};
...
- **Construction Director Creation**: active - Fully implemented with approval workflow integration
- **Construction Director Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Construction Director Creation
interface construction_director_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'construction_director_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in construction_director approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Construction Director Creation
...
### 4.1 Construction Director Creation Modal

**Modal ID**: `ConstructionDirectorCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new construction director records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 00882-2026-00001]
...
### 4.2 Construction Director Details Modal

**Modal ID**: `ConstructionDirectorDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit construction director record details
...
**Purpose**: Store and retrieve construction director documents and attachments

**Integration Points**:
- **Document Upload**: Upload construction director related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/construction_director/documents/upload
/api/construction_director/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for construction director record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of construction director status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/construction_director/email/status-update
/api/construction_director/email/approval-request
```
...
#### Construction Director Table Component
**Component**: `Construction DirectorTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Construction Director Record Validation
```javascript
const construction_directorRecordValidationRules = {
  construction_directorId: {
    required: true,
...
- **Record Not Found**: "Construction Director record not found. Please check the record ID."
...
#### Lazy Loading
- Load construction director records and data on demand
- Paginate large construction director records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used construction director records and data for 24 hours
...
- Full-text search across construction director names and descriptions
- Fuzzy matching for record details
- Category-based filtering for construction director records
...
#### Construction Director Workflow Testing
...
- **Agent**: Director (KnowledgeForge AI)
- **Primary Discipline**: 00882 Construction Director
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - construction-director-knowledgeforge
  - construction-director-knowledgeforge-construction-director
...
|----------|--------|---------|----------|
| `/api/construction_director/records` | GET | List construction director records | Array of record objects |
| `/api/construction_director/details` | GET | Get construction director record details | Record detail object |

### Appendix B: Common Construction Director Abbreviations

| Abbreviation | Full Term |
|-------------|-----------|
| 0088 | Construction Director |
| REC | Record |

### Appendix C: Construction Director Record Number Format

**Format**: 00882-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 00882-2026-00001 (First construction director record of 2026)
- 00882-2026-01234 (1234th construction director record of 2026)
...
**Container**: `A-00882-button-container`
...
- Page Load State: `construction_director-baseline-page-load.png`
- Button Interactions: `construction_director-baseline-button-states.png`
- Modal Layouts: `construction_director-baseline-modals.png`
- Form States: `construction_director-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('construction_director-correspondence-btn');
...
**Applicable To**: Director Agent - KnowledgeForge AI
