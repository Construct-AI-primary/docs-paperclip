---
title: Public Relations Agent Architectural Knowledge
description: Comprehensive architectural knowledge of public relations system design, database schemas, API patterns, and compliance frameworks for public relations agents
author: KnowledgeForge AI
version: 1.0
memory_layer: durable_knowledge
para_section: docs_paperclip/disciplines/02100-public-relations
gigabrain_tags: knowledgeforge-ai, public-relations-agent, public-relations-architecture, system-design, compliance-frameworks
openstinger_context: public-relations-agent-architecture, public-relations-system-design
last_updated: 2026-04-10
related_docs:
  - docs-construct-ai/disciplines/02100_Public Relations/agent-data/domain-knowledge/02100_DOMAIN-KNOWLEDGE.MD
  - docs-construct-ai/disciplines/02100_Public Relations/02100-public-relations-page-documentation.md
  - docs-paperclip/companies/knowledgeforge-ai/skills/public-relations-knowledgeforge/SKILL.md
---
...
This document provides the Specialist agent with comprehensive architectural knowledge of the public relations system design, database schemas, API patterns, security frameworks, and compliance requirements. The Specialist agent serves as the architectural authority for public relations module implementation, ensuring adherence to coding standards, EU regulations, privacy requirements, and quality assurance frameworks.

**Purpose**: Enable the Specialist agent to guide public relations system architecture, validate design decisions, enforce compliance standards, and provide architectural consultation to development teams within the Construct AI ecosystem.

---

## Part 1: Public Relations Database Architecture
...
#### Public Relations Records Table Architecture
```sql
-- Supabase table with RLS policies
CREATE TABLE public_relations_records (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  public_relations_id VARCHAR(20) UNIQUE NOT NULL,
...
ALTER TABLE public_relations_records ENABLE ROW LEVEL SECURITY;

-- RLS Policies for multi-tenant access
CREATE POLICY "public_relations_records_company_access" ON public_relations_records
...
CREATE INDEX idx_public_relations_records_public_relations_id ON public_relations_records(public_relations_id);
CREATE INDEX idx_public_relations_records_status ON public_relations_records(status);
CREATE INDEX idx_public_relations_records_company_id ON public_relations_records(company_id);
```
...
```sql
CREATE TABLE public_relations_details (
  name VARCHAR(255) PRIMARY KEY,
  value TEXT NOT NULL REFERENCES public_relations_records(public_relations_id),
...
CREATE INDEX CONCURRENTLY idx_public_relations_records_composite
ON public_relations_records(status, status, created_at DESC);

-- Partial indexes for active records
CREATE INDEX idx_public_relations_records_active
ON public_relations_records(public_relations_id)
...
CREATE TABLE public_relations_records_y2026m01 PARTITION OF public_relations_records
FOR VALUES FROM ('2026-01-01') TO ('2026-02-01');
```

#### Caching Strategy
- Redis cache for frequently accessed public relations records and data
- Application-level caching with TTL: {cache_ttl_hours} hours
- Cache invalidation: Event-driven cache clearing

---

## Part 2: Public Relations API Architecture

### 2.1 RESTful API Design Patterns

**Base URL**: `/api/public-relations`
...
**Versioning**: URL-based versioning (`/api/v1/public-relations`)

#### Core API Endpoints

**Public Relations Record Management**:
```typescript
// GET /api/public-relations/public relations records
// List public relations records with filtering and pagination
{
  "data": [{entity_schema}],
  "pagination": {
    "page": number,
...
// POST /api/public-relations/public relations records
...
// PUT /api/public-relations/public relations records/{id}
...
const PERMISSIONS = {
  'create_public_relationsRecord': ['public-relations_admin', 'public-relations_manager'],
  'approve_public_relationsRecord': ['public-relations_admin'],
  'view_public_relationsRecord': ['public-relations_user', 'public-relations_admin', 'public-relations_manager']
};
```

#### Input Validation & Sanitization
```typescript
// Zod Schema for Public Relations Record
const public_relationsRecordSchema = z.object({
  public_relations_id: z.string().min(1).max(50),
...
const sanitizedQuery = sql`
  SELECT * FROM public_relations_records
  WHERE public_relations_id = ${userInput}
...
const cacheConfig = {
  ...
  keyPrefix: 'public-relations:',
  compression: 'gzip'
};

// Cache Keys Pattern
const CACHE_KEYS = {
  public_relationsRecord_list: ({filters}) => `public-relations:public relations records:list:${hash(filters)}`,
  public_relationsRecord_detail: (id) => `public-relations:public_relationsRecord:${id}`,
  public_relationsRecord_stats: () => `public-relations:public relations records:stats`
};
```
...
EXPLAIN ANALYZE
SELECT public_relations_id, status, status, created_at
FROM public_relations_records
WHERE status IN ('active', 'pending')
  AND created_at >= $1
...
-- Index Scan using idx_public_relations_records_status_created_at
-- No sequential scans on large tables
```

---

## Part 3: Public Relations Security Architecture
...
// Permission Matrix
const RBAC_MATRIX = {
  'public-relations_admin': {
    create: ['public relations records', 'users', 'reports'],
    read: ['all'],
    update: ['all'],
    delete: ['public relations records', 'reports'],
    approve: ['public relations records', 'budgets']
  },
  'public-relations_manager': {
    create: ['public relations records', 'reports'],
    read: ['public relations records', 'reports', 'users'],
    update: ['public relations records', 'reports'],
    delete: [],
    approve: ['public relations records']
  },
  'public-relations_user': {
    create: ['public relations records'],
    read: ['public relations records', 'reports'],
    update: ['own_public relations records'],
    delete: [],
    approve: []
  }
};
...
- **Public Relations Creation**: active - Fully implemented with approval workflow integration
- **Public Relations Processing**: active - Record processing with automated validation

**Workflow Implementation Details**:
```typescript
// Actual implementation of Public Relations Creation
interface public_relations_creation_workflowImplementation {
  steps: WorkflowStep[];
...
const WORKFLOW_STATUS = {
  'public_relations_creation_workflow': {
    status: 'active', // 'active', 'deprecated', 'maintenance'
    version: '2.1.3',
    lastUpdated: '2026-03-15',
    knownIssues: ['ISSUE-001: Race condition in public_relations approval routing'],
    performance: {
...
**Workflow Test Cases**:
```typescript
// Comprehensive test scenarios for Public Relations Creation
...
### 4.1 Public Relations Creation Modal

**Modal ID**: `PublicRelationsCreationModal`
**Trigger**: "New Record" button
**Purpose**: Create new public relations records

**Form Sections**:

1. **Basic Information**
   ```
   Record ID: [Auto-generated: 02100-2026-00001]
...
### 4.2 Public Relations Details Modal

**Modal ID**: `PublicRelationsDetailsModal`
**Trigger**: "View Details" button
**Purpose**: View and edit public relations record details
...
**Purpose**: Store and retrieve public relations documents and attachments

**Integration Points**:
- **Document Upload**: Upload public relations related documents
- **Document Access**: Retrieve documents for record processing

**API Endpoints**:
```
/api/public_relations/documents/upload
/api/public_relations/documents/get
```

### 5.2 Email Integration

**System**: Email notification system
**Purpose**: Send notifications for public relations record updates

**Integration Points**:
- **Status Updates**: Notify stakeholders of public relations status changes
- **Approval Requests**: Send approval requests to designated approvers

**API Endpoints**:
```
/api/public_relations/email/status-update
/api/public_relations/email/approval-request
```
...
#### Public Relations Table Component
**Component**: `Public RelationsTable`
**Features**:
- Sortable columns
- Filterable data
- Pagination
...
#### Public Relations Record Validation
```javascript
const public_relationsRecordValidationRules = {
  public_relationsId: {
    required: true,
...
- **Record Not Found**: "Public Relations record not found. Please check the record ID."
...
#### Lazy Loading
- Load public relations records and data on demand
- Paginate large public relations records lists
- Load detailed information only when needed

#### Caching Strategy
- Cache frequently used public relations records and data for 24 hours
...
- Full-text search across public relations names and descriptions
- Fuzzy matching for record details
- Category-based filtering for public relations records
...
#### Public Relations Workflow Testing
...
- **Primary Discipline**: 02100 Public Relations
- **Knowledge Type**: Page-Specific UI and Workflow Knowledge
- **Last Reviewed**: 2026-04-10
- **Version**: 1.0
- **Related Skills**:
  - public-relations-knowledgeforge
  - public-relations-knowledgeforge-public-relations
...
|----------|--------|---------|----------|
| `/api/public_relations/records` | GET | List public relations records | Array of record objects |
| `/api/public_relations/details` | GET | Get public relations record details | Record detail object |

### Appendix B: Common Public Relations Abbreviations

| Abbreviation | Full Term |
|-------------|-----------|
| 0210 | Public Relations |
| REC | Record |

### Appendix C: Public Relations Record Number Format

**Format**: 02100-{YYYY}-{NNNNN}
- YYYY: 4-digit year
- NNNNN: 5-digit sequential number (padded with zeros)

**Examples**:
- 02100-2026-00001 (First public relations record of 2026)
- 02100-2026-01234 (1234th public relations record of 2026)
...
**Container**: `A-02100-button-container`
...
- Page Load State: `public_relations-baseline-page-load.png`
- Button Interactions: `public_relations-baseline-button-states.png`
- Modal Layouts: `public_relations-baseline-modals.png`
- Form States: `public_relations-baseline-forms.png`

**Layout Validation Tests**:
```javascript
// Test button positioning accuracy
test('button positioning within tolerance', () => {{
  const button = screen.getByTestId('public_relations-correspondence-btn');
